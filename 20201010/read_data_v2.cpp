...
*/
    ...
    if(input == 'g') {
        double k;
        cout << "Choose the Frequency Number(1 -> 1.7MHz, 2 -> 3.4MHz) : ";
        cin >> k;
        start = chrono::system_clock::now();
        double **voltage_13 = new double*[16];
        double **voltage_15 = new double*[16];
        goertzel_method(buf13, voltage_13, k, 0);
        goertzel_method(buf15, voltage_15, k, 0);
...
*/
    ...
    const double gain13[16] = //BBGC
    {1.000000000000000000e0, 9.864749932865497506e-1, 1.008554910983899822e0, 9.909215189952645941e-1, 
    ...
    const double gain15[16] = //BBGC
    {1.000000000000000000e0, 9.927984352486749486e-1, 9.886965960947093901e-1, 9.844666881364172450e-1, 
...
void dot_matrix(double ** _input_double, double ** _output, char address, char method) {
    if (address == 13) {
        double monitor_matrix[16][16] = //WC
...
void extract_momentum(double ** _input_double13, double ** _input_double15, int order, int bunch, char method) {
    ...
    const double align13[2] = {0.7799954861648153, -0.17114181002093012}; //BBA
    const double align15[2] = {-0.25069580086359916, 1.0363901454673603}; //BBA
